...
if __name__ == '__main__':
    ...
    test_set = 'coco_2017_test-dev' #'voc_2007_test' or 'voc_2007_test' or 'voc_2012_test' or 'coco_2014_minival' or 'coco_2015_test-dev'
    voc_path = 'models/VGGNet/VOC0712/refinedet_vgg16_320x320/'
    #coco_path = 'models/VGGNet/coco/refinedet_vgg16_320x320/'
    model_name = 'refinedet_xception_320x320'
    coco_path = 'trained_models/' + model_name + '/'
    ...
    models = os.listdir(path_final)

    mAP = {}
    for model in models:
        if model.find('caffemodel') == -1:
            continue
        caffemodel = path_final + model
        
        if '400' in model:
            mode = '400'
        if '400_reduced' in model:
            mode = '400_reduced'
        if '1000' in model:
            mode = '1000'
        if '1000_reduced' in model:
            mode = '1000_reduced'
	
        if 'coco' in test_set:
	    prototxt = path + 'deploy_' + mode  + '.prototxt'
            f = open(prototxt, 'r')
            for line in f:
    ...
    if 'voc' in test_set:
        ...
        for key in keys:
            ...
            templine.append("%d\t    %.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\n"
                            %(key,value[0],value[1],value[2],value[3],value[4],value[5],value[6],value[7],value[8],value[9],value[10],value[11]))
        mAP_path = 'test/mAPs/' + model_name + '_' + mode + '_mAP.txt'
